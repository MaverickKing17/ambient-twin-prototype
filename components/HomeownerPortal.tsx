...
const Icons = {
  ...
  // Added Activity icon to resolve Property 'Activity' does not exist error
  ...
  if (accessState === 'verifying') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0b1120] text-white">
         ...
         <p className="text-slate-500 text-xs font-mono">ENCRYPTED ID: {homeId.toUpperCase()}</p>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto space-y-8 pb-32 animate-fade-in relative z-10 px-4">
      ...
      <div className="flex items-center justify-between py-6">
        <div className="flex items-center gap-3">
           <div className="h-8 w-8 rounded bg-blue-600 flex items-center justify-center text-white font-bold text-xs shadow-lg">A</div>
           <span className="text-lg font-bold text-white tracking-tight">Ambient Portal</span>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-3 py-1 rounded bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] font-black uppercase tracking-widest">
            <Icons.Shield />
      ...
      {/* REBATE HERO */}
      <GlassCard variant="mica" className="text-center py-16 border-t-4 border-t-blue-500 relative overflow-hidden group">
        <div className="absolute inset-0 bg-gradient-to-b from-blue-500/5 to-transparent pointer-events-none" />
        <span className="text-xs font-black text-slate-500 uppercase tracking-[0.4em] mb-4 block">Official 2026 Audit Result</span>
        ...
        <div className="flex flex-col items-center gap-1">
           ...
           <span className="text-xs text-slate-500 uppercase font-black tracking-widest">Pre-Verified via Digital Twin</span>
        </div>
      </GlassCard>

      {/* ANALYTICS SECTION */}
      <GlassCard title="Projected Resource Waste Analysis" icon={<Icons.Activity />} variant="mica">
         <div className="mb-8 p-4 bg-red-500/5 border border-red-500/10 rounded">
            <p className="text-sm text-slate-300 leading-relaxed">
              Your current <span className="text-white font-bold">{profile.system}</span> is losing <span className="text-red-400 font-bold">${profile.monthlyWaste}/mo</span> in mechanical waste. 
              <span className="text-blue-400 font-bold ml-1 cursor-default">Select a month below to view diagnostic specifics.</span>
            </p>
         </div>
         ...
         <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} onClick={(v) => v && v.activeTooltipIndex !== undefined && setSelectedMonthIndex(v.activeTooltipIndex)} cursor="pointer">
                ...
                <YAxis stroke="rgba(255,255,255,0.3)" fontSize={11} axisLine={false} tickLine={false} tickFormatter={(v) => `$${v}`} />
                <Tooltip cursor={{fill: 'rgba(255,255,255,0.02)'}} contentStyle={{ backgroundColor: '#161d2e', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px' }} />
                <Bar dataKey="current" name="Waste Profile" fill="#3b82f6" radius={[4, 4, 0, 0]} barSize={24}>
                   {chartData.map((e, i) => <Cell key={i} fillOpacity={selectedMonthIndex === i ? 1 : 0.4} fill={selectedMonthIndex === i ? '#3b82f6' : '#94a3b8'} />)}
                </Bar>
         ...
         <div className={`mt-8 transition-all duration-500 ${activeMonth ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}`}>
            {activeMonth && (
               <div className="p-6 bg-white/[0.02] border border-white/5 rounded-lg flex flex-col md:flex-row items-center gap-6 animate-fade-in">
                  ...
                  <div className="flex-1">
                     <div className="text-[10px] font-black uppercase text-blue-400 tracking-[0.2em] mb-1">{activeMonth.name} Technical Diagnosis</div>
                     <h3 className="text-xl font-bold text-white mb-1">{activeMonth.reason}</h3>
                     <p className="text-xs text-slate-400">Sub-optimal system pressure is causing over-cycle conditions, increasing hydro draw by <span className="text-white font-bold">${activeMonth.current - activeMonth.optimized}</span> this month.</p>
                  </div>
               </div>
            )}
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         <div className="p-8 bg-white/[0.02] border border-white/5 rounded-xl flex flex-col justify-between hover:bg-white/[0.04] transition-all">
            <div className="space-y-3 mb-8">
               <h4 className="text-xl font-bold text-white">Full Energy Report</h4>
               <p className="text-sm text-slate-400 leading-relaxed">Download your blockchain-verified technical addendum for your home appraisal or realtor files.</p>
            </div>
            <button className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded text-xs font-bold uppercase tracking-widest text-white flex items-center justify-center gap-3 transition-all">
               <Icons.Download />
               Get PDF Summary
            </button>
         </div>

         <div className="p-8 bg-blue-600 border border-blue-400/30 rounded-xl flex flex-col justify-between shadow-2xl shadow-blue-900/40 group hover:bg-blue-500 transition-all">
            <div className="space-y-3 mb-8">
               <h4 className="text-xl font-bold text-white">Secure Rebate Lock</h4>
               <p className="text-sm text-blue-100 leading-relaxed">Schedule your pre-retrofit verification with <strong>{profile.contractor}</strong> to finalize your $10,600 claim.</p>
            </div>
            <button 
               onClick={handleConsult}
               disabled={sent || isSending}
               className={`w-full py-4 rounded text-xs font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all
                  ${sent ? 'bg-emerald-500 text-white cursor-default' : 'bg-white text-blue-600 hover:scale-[1.02]'}
               `}
            >
